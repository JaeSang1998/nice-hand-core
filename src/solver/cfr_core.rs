// CFR 알고리즘 핵심 구현
// Preference CFR (δ/ε) 를 사용한 포커 전략 학습
...
impl<G: Game> Trainer<G> {
    ...
    pub fn run(&mut self, roots: Vec<G::State>, iterations: usize) {
        ...
        println!("📚 CFR 학습 시작 - {} 시나리오, {} 반복", roots.len(), iterations);
        ...
        let result = if let Some(player) = G::current_player(state) {
            ...
            if actions.is_empty() {
                ...
                for (i, &action) in actions.iter().enumerate() {
                    ...
                    utilities[i] = self.cfr_with_depth(&next_state, hero, prob * strategy[i], rng, depth + 1);
                    node_util += strategy[i] * utilities[i];
                }
                
                // 히어로 플레이어면 리그렛과 전략 합계 업데이트
                if player == hero {
