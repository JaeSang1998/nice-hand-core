...
fn demo_tournament_cfr_integration() {
    ...
    // Create game state for CFR training
    let mut game_state = GameState::new(4);
    game_state.pot = 300;
    game_state.current_bet = 150;
    game_state.players[0].stack = player_stacks[0];
    game_state.players[1].stack = player_stacks[1];
    game_state.players[2].stack = player_stacks[2];
    game_state.players[3].stack = player_stacks[3];
    
    // Initialize CFR solver with tournament awareness
    let mut cfr_solver = CFRSolver::new();
    ...
    for iteration in 1..=50 {
        ...
        let _result = cfr_solver.cfr(&mut game_state, 1.0, 1.0, iteration);
...
// Helper function to create a realistic game state
impl GameState {
    fn new(num_players: usize) -> Self {
        ...
        for i in 0..num_players {
            players.push(Player {
                ...
                folded: false,
                all_in: false,
                cards: vec![], // Would be populated in real game
            });
        }
