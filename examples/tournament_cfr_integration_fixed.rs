...
fn demo_tournament_cfr_integration() {
    ...
    for iteration in 1..=50 {
        ...
        if iteration % 10 == 0 {
            println!(
                "      Iteration {}: Tournament-aware strategy updates",
                iteration
            );
        }
    }
...
fn demo_realtime_tournament_decisions() {
    ...
    let player_stacks = vec![
        18000, 15000, 12000, 8000, 6000, 5000, 4000, 3000, 2000, 1500,
    ];
    ...
    for (description, player_idx, stack_ratio, position) in decision_scenarios {
        ...
        let recommended_action =
            evaluator.select_opponent_action(player_idx as u32, &context, &available_actions);
    ...
    println!(
        "      Decision speed: {:.2}μs per decision",
        decision_time.as_micros() as f64 / 1000.0
    );
    println!(
        "      Throughput: {:.0} decisions/second",
